...
const TDD_OFF_PROMPT = [
  "[TDD MODE — OFF]",
  "TDD mode enforces test-driven development (specifying → implementing → refactoring). Before enabling TDD: scaffold the project first — create config files (package.json, pyproject.toml, Cargo.toml, go.mod, etc.), install the test framework, and ensure the test command works (even if there are no tests yet). Then call tdd_start to begin TDD. Do not use TDD for config changes, documentation, scaffolding, or exploratory tasks.",
].join("\n");
